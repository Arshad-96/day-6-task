class uberPrice {
  ...
  calculatePrice(totalDistance, totalTime, totalWaitingTime) {
    return (
      this.baseFare +
      this.ratePerKm * totalDistance +
      this.ratePerMin * totalTime +
      this.mainFareSupplement +
      this.waitingPerMin * totalWaitingTime +
      this.shareAdjustment
    );
  }
}

//fixed fares;
const ratePerKm = 0.64;
const ratePerMin = 0.136;
const waitingPerMin = 0.208;
const baseFare = 0.84;
const mainFareSupplement = 0.62;
const shareAdjustment = 0.08;

//creating fixed object
const basePrice = new uberPrice(
  ratePerKm,
  ratePerMin,
  waitingPerMin,
  baseFare,
...
let totalPrice = basePrice.calculatePrice(
  totalDistance,
  totalTime,
  totalWaitingTime
);
